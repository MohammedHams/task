...
<input type="hidden" name="order_num" value="{{$order_num}}">
 ...
 @endif  
  ...
  <div class="form-group col-sm-12 ">
                <label class="col-md-4 control-label color-view bg-blue-dark bg-font-blue-dark"> القسم   </label>
                <div class="col-md-8">

                    <select id="dep"  name="dep" class="form-control select2 " >
                      <option value="">اختر   القسم   المصروف له  الطلبية    </option>

                        @foreach($npla_departments as $npla_department)
                      <option value="{{$npla_department->id }}">{{$npla_department->name}}</option>
    ...
    <input type="hidden" id="store_keeper" name="store_keeper" value="1">
    ...
    <button type="submit" class="btn green btn-sm">حفظ</button>
</form>
 ...
 show_data();}
  ...
  function show_data(){
    $('#sample_1').DataTable({
     ...
     "pageLength": 10,
     "ajax": {
            "url": '{!! URL::asset("/stock_store/order_data_storekeeper") !!}',
          "type": "GET",
            "data": function ( d ) {
...
//alert(data);
$('#required_item').append('<tr><td><input  class="r_item_id_pk" name="r_item_id_pk[]" type="checkbox"  value="'+ id.replace(/\"/g, "")+','+name.replace(/\"/g, "")+'" onclick="remove_order($(\'.r_item_id_pk\').index(this),this)"></td> <td> '+name.replace(/\"/g, "")+'</td><td><input type="number" value="'+number.replace(/\"/g, "")+'" class="number_needed form-control input-xsmall" name="number_needed[]"> <input type="hidden" value="'+id.replace(/\"/g, "")+'" class="item_needed" name="item_needed[]"></td> <td><input type="text" class="note_needed form-control input-xsmall" name="note_needed[]"></td> </tr>');
...
$("#add_all_order").validator().on('submit', function(e) {
 ...
 e.preventDefault();             
    ...
    $.ajax({
     url:" {!! URL::asset('/stock_store/items/confirm_order_data') !!}",
