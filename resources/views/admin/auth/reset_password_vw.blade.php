...
<html lang="en" dir="rtl">
    ...
    <body class=" login">
        ...
        <div class="content">
            ...
            <!-- BEGIN LOGIN FORM -->
            <form class="form-horizontal" method="post"  id="reset_password" enctype="multipart/form-data">
           {{ csrf_field() }}

                <h3 class="form-title font-green" style="font-family: 'DroidArabicKufiRegular', sans-serif;"> تعديل  كلمة  المرور   </h3>
                <div class="alert alert-danger" id="reset_password_alert" style="display:none" >
                        <ul id="reset_password_error"> 
                        </ul>
                        </div>
                ...
                <div class="form-group">
                    <label class="control-label visible-ie8 visible-ie9">كود  التحقق   </label>
                    <input class="form-control form-control-solid placeholder-no-fix" type="text" autocomplete="off" placeholder="كود  التحقق " name="true_code" required="required" /> </div>


                      <div class="form-group">
                    <label class="control-label visible-ie8 visible-ie9"> كلمة  المرور  الجديدة  </label>
                    <input class="form-control form-control-solid placeholder-no-fix" type="password" autocomplete="off" placeholder="كلمة  المرور  الجديدة " name="password" required="required" /> </div>


                <div class="form-actions">
                    <button type="submit" class="btn green uppercase">تعديل  كلمة  المرور  </button>
                    <br/>
                      <br/>
...
$("#reset_password").on('submit', function(e) {
  ...
  e.preventDefault(); 
         ...
         $.ajax({
                type: 'POST',
                dataType: "json",
                url: '{!! URL::asset("user/reset_password") !!}',
            ...
            error: function(e) 
            {
              ...
              $('#reset_password_error').empty();
              var error = e.responseJSON;
              $.each(error, function (i, member) {
              for (var i in member) {
              $('#reset_password_error').append('<li >'+ member[i] +'</li>' );
             }
           });
